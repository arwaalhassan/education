...
exports.addCourse = (req, res) => {
  ...
  const sql = 'INSERT INTO islamic_courses (title, description, trainer, price) VALUES (?, ?, ?, ?)';
...
exports.getCourses = (req, res) => {
  const sql = 'SELECT * FROM islamic_courses';
...
exports.deleteCourse = (req, res) => {
  ...
  const sql = 'DELETE FROM islamic_courses WHERE id = ?';
...
exports.addSchedule = (req, res) => {
  ...
  const sql = 'INSERT INTO islamic_schedule (course_id, day_of_week, start_time, end_time, hall_id) VALUES (?, ?, ?, ?, ?)';
...
exports.getSchedule = (req, res) => {
  const sql = `
    ...
    FROM islamic_schedule ts
    JOIN islamic_courses tc ON ts.course_id = tc.id
...
exports.deleteSchedule = (req, res) => {
  const { id } = req.params;
  const sql = 'DELETE FROM islamic_schedule WHERE id = ?';
  db.query(sql, [id], (err, result) => {
    if (err) return res.status(500).json({ error: 'DB error' });

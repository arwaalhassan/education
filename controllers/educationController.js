...
exports.addCourse = (req, res) => {
  ...
  const sql = 'INSERT INTO education_courses (title, description, trainer, price) VALUES (?, ?, ?, ?)';
...
exports.deleteCourse = (req, res) => {
  ...
  const sql = 'DELETE FROM education_courses WHERE id = ?';
...
exports.getCourses = (req, res) => {
  const sql = 'SELECT * FROM education_courses';
...
exports.addSchedule = (req, res) => {
  ...
  const sql = 'INSERT INTO education_schedule (course_id, day_of_week, start_time, end_time, hall_id) VALUES (?, ?, ?, ?, ?)';
...
exports.getSchedule = (req, res) => {
  const sql = `
    ...
    FROM education_schedule ts
    JOIN education_courses tc ON ts.course_id = tc.id
...
exports.deleteSchedule = (req, res) => {
  const { id } = req.params;
  const sql = 'DELETE FROM education_schedule WHERE id = ?';
  db.query(sql, [id], (err, result) => {
    if (err) return res.status(500).json({ error: 'DB error' });
